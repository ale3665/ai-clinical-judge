...
> I built an AI Judge evaluation layer for a clinical concept extraction AI Labeler. The Judge compares Labeler outputs against gold-standard labels, checks span correctness, concept type, normalization, evidence support, hallucinations, missing concepts, and clinical risk. It produces model-level metrics such as strict F1, soft F1, hallucination rate, missing concept rate, and high-risk error rate, and it generates a review queue for clinician validation.
